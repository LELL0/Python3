...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.smartCoinFlip():
            for g in reversed(WOFComputerPlayer.SORTED_FREQUENCIES):
                if g not in guessed:
                    if g in VOWELS and self.prizeMoney >= VOWEL_COST:
                        return g
                    elif g not in VOWELS:
                        return g
        else:
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        time.sleep(1)
        print("{}'s guess ==> [{}]".format(self.name, guess))
        time.sleep(3)
        return guess
